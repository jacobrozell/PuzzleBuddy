...
`PuzzleBuddy.xcodeproj` is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen) from `project.yml` and is **not** committed. After every pull:
...
- **No `print()` in app code** — custom rule `no_print_statements` warns in `App/**/*.swift`. Use `AppLog` instead.
